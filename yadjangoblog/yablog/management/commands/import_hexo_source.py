...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        files = sorted(
            list(filter(lambda x: x.endswith(".md"), map(lambda x: hexo_path + '/' + x, os.listdir(hexo_path)))))
        ...
        for idx, blogpost in enumerate(exist_blogposts):
            if blogpost.title in need_update_titles:
                file_status = list(filter(lambda x: x["title"] == blogpost.title, file_status_list))
                if file_status:
    ...
    def process_blogpost(self, file):
        with open(file, "rt") as f:
            ...
            blogcategory, created = BlogCategory.objects.get_or_create(name=article_info["categories"][0])
            if created:
                blogcategory.save()
            ...
            blogpost.publish_date = timezone.make_aware(article_info["date"], timezone.get_current_timezone())
